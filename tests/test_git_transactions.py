...
class GitTests(unittest.TestCase):
    ...
    def test_v5_and_v6_epochs_can_be_published_but_development_epochs_cannot(self):
        from agent_preprints import PROTOCOL_V5, PROTOCOL_V6, poi, poi_v6, taxonomy
        ...
        for protocol, engine in ((PROTOCOL_V5, poi), (PROTOCOL_V6, poi_v6)):
            with self.subTest(protocol=protocol):
                git(self.seed, "pull", "--ff-only", "origin", "main")
                initialize(self.seed, read_json(source), "test/archive", "1", "https://test.github.io/archive/", protocol)
                taxonomy.ensure(self.seed)
                git(self.seed, "add", ".")
                git(self.seed, "commit", "-m", "Configure " + protocol + " fixture")
                git(self.seed, "push", "origin", "main")
                checkout = self.clone(protocol + "-epoch-writer")
                tx = GitTransaction(checkout, "main")
                commit, epoch = tx.run(lambda tree: rotate(tree, NOW))
                self.assertEqual(epoch["poa_policy"], engine.PRODUCTION_POLICY)
                names = git(self.remote, "diff-tree", "--no-commit-id", "--name-only", "-r", commit).splitlines()
                self.assertIn("challenges/epochs/" + protocol.rsplit("-", 1)[1] + "-2026-09-17.json", names)
                with self.assertRaises(Rejection) as caught:
                    tx.run(lambda tree: rotate(tree, NOW, development=True, protocol=protocol))
                self.assertEqual(caught.exception.code, "unsafe_write")
